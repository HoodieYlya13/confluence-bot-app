...
- **RBAC playground** (`/playground`) — ask one question through two real MCP sessions holding different bearer tokens (`JUNIOR_OP` vs `ATS_CORE_LEAD`), in two modes: **Compare retrieval** (`semantic_search_accelerator`) shows the document chunks each level may retrieve, with chunks withheld from the junior highlighted; **Compare answers** (`ask_accelerator_operations`) runs the full LangGraph agent server-side and shows one grounded answer per role, with restricted register addresses surfaced only in the lead's answer.
