...
The desktop client (`confluence-spotlight`) can sign in through this console rather than shipping a token, so the same `MCP_TOKEN_*` that power the playground also serve the desktop app — and the bearer token never lands in the desktop *webview*, mirroring the `server-only` posture here. This is the **demo / dev** sign-in path: a release desktop build instead authenticates against the self-hosted OpenID Connect provider (`auth.hy13dev.com`) for a real per-account access + refresh token, and the MCP server accepts both kinds of bearer against one RBAC model. Three pieces implement the console's standard public-client OAuth shape:

- **`app/spotlight-login/page.tsx`** — the authorize/login surface. A server component that validates `redirect_uri` against an allowlist (`isAllowedRedirectUri`, covering both `confluence-spotlight://auth` and the local-beta build's `confluence-spotlight-beta://auth`) and `code_challenge`/`state`, then renders one persona button per configured role. This is *where role selection belongs* — at the IdP, not baked into the client.
...
Mode is a second search param (`?mode=search|answer`, default `search`), carried by a segmented radio toggle (`name="mode"`, two `<input type="radio">` each styled via Tailwind's `peer` + `peer-checked:` on a sibling `<span>` — sky for retrieval, violet for answers) and one shared submit button. `peer-checked:` is used rather than `has-[:checked]:` because the latter, while it compiles, was rewritten to invalid `has-:checked:` (no generated CSS) by format-on-save in this repo; `peer-checked:` is a bracket-free core variant a formatter leaves intact. Each result footer links to the same query in the other mode. Answer mode is rate-limited tighter than search (4/min, 150/day vs 10/min, 500/day) because it spends an LLM call server-side, not just a vector query.

The toggle is a **radio field, not a second submit button**, for a non-obvious reason. The first attempt used two submit buttons sharing `name="mode"` (values `search`/`answer`), relying on the clicked button's `name`/`value` being encoded into the URL. That works for a native no-JS form, but `next/form`'s client-side enhancement builds the destination URL with `new FormData(formElement)` (see `next/dist/esm/client/form-shared.js`), and the standard `FormData` constructor **omits the submitter button** unless it is passed explicitly — which Next does not. So with JS enabled (the common case) `mode` was silently dropped and every submit defaulted to `search`. `<Form>` itself documents the fix: extra params must come from a real form field, not the submitter. A checked radio is a real field, so it is included on both the JS and no-JS paths; the radios are keyed to `mode` so the toggle reflects the URL after a footer-link mode switch (uncontrolled inputs otherwise ignore `defaultChecked` on soft navigation).
...
Each retrieved chunk now carries a `url` (the server resolves it from the Confluence `_links.webui` slug) and a human-readable `title`. In retrieval mode the chunk's title becomes a sky link to that page; in answer mode the agent appends a `**Sources:**` footer of Markdown links labelled by title, so the playground closes the loop from "here is the grounded answer" to "here is the page it came from". The link **text is the page title, not the `doc_id`** — against live Confluence the `doc_id` is an opaque numeric page id (`983042`), which reads as noise; the title is what a reviewer recognises. Both fields are optional and the label falls back to `doc_id` when a title is absent. This required teaching the hand-rolled `parseMarkdown` inline pass to recognise `[text](url)` (it previously handled only bold, inline code and hex tokens) — links open in a new tab with `rel="noopener noreferrer"`. Against the air-gapped local source the server sends no URL (and the mock pages carry no title key), so the label renders as plain text `doc_id`, and the console degrades cleanly when pointed at an offline server.
